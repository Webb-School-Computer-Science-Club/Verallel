...
{
	switch(type)
	{ // Uses switch because more types will be added in the future for different class tabs
			
		case 0:
			var lin = document.getElementById('better-link-class-home');
			if (lin == null)
			{ //Link shouldn't appear twice
...
chrome.tabs.onUpdated.addListener(function(tabId, changeInfo, tab)
{
	...
	{
		if (tab.url.match(/assignment/) && tab.url.match(/detail/))
		{ // Specific part of the domain improve is improving
			chrome.scripting.executeScript({target: {tabId: tabId}, func: improve, args: [1]}); // For assignment detail
		}
		else if (tab.url.match(/submit-assignments/))
		{ // Dropbox improvement (more features are to be added!)
